...
while True:
    ...
    for i in range(len(scores)):
        if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)):
            ...
            if object_name in vehicle_classes:
                ...
                GPIO.output(in4, GPIO.LOW)
                GPIO.output(in3, GPIO.HIGH)
                sleep(0.5)  # Adjust the time for the maneuver
                ...
                GPIO.output(in4, GPIO.LOW)
                GPIO.output(in3, GPIO.HIGH)
                sleep(2)  # Adjust the time for moving forward
    ...
    # Lógica de seguimiento de carril
    angle = carril.follow_lane(frame)
    print(f"Detected angle: {angle}")
    
    # Convert the angle to motor control signals
    if angle < -10:  # Turn left
        GPIO.output(in1, GPIO.LOW)
        GPIO.output(in2, GPIO.HIGH)
        GPIO.output(in4, GPIO.HIGH)
        GPIO.output(in3, GPIO.LOW)
    elif angle > 10:  # Turn right
        GPIO.output(in1, GPIO.HIGH)
        GPIO.output(in2, GPIO.LOW)
        GPIO.output(in4, GPIO.LOW)
        GPIO.output(in3, GPIO.HIGH)
